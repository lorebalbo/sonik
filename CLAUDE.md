...
- **Design tokens:** every colour, border width and font in UI code comes from `Source/Features/Shared/Ui/SonikTheme.h` (`sonik::ui::theme`). Hex colour literals in component files are a defect.
- **Shared painters:** dithered fills, dithered drop shadows and the canonical DESIGN.md latch button are painted via `Source/Features/Shared/Ui/SonikDraw.h` (`sonik::ui::draw::fillDithered`, `fillDitheredMeter`, `drawDitheredShadow`, `paintLatchButton`). Never reimplement these per component.
- **Typography:** Space Mono Regular/Bold (SIL OFL) is embedded in the binary via `SonikBinaryData` (`Assets/Fonts/`). `SonikLookAndFeel::getTypefaceForFont` routes EVERY font request to it, so the family renders identically on machines without the font installed. Request fonts via `sonik::ui::theme::mono (height, bold)`.
- **Interaction feedback (mandatory):** all clickable controls show instant hover feedback (inactive fill steps to `containerHighest`, no fades) and the pointing-hand cursor; disabled controls render ink at `theme::kDisabledAlpha` with the normal cursor. `juce::Button` subclasses get hover from `paintButton`'s `isMouseOver`; plain `juce::Component` controls call `setRepaintsOnMouseActivity (true)`.
- **Popup menus, combo boxes, text buttons, scrollbars, tooltips, text editors and alert windows** are styled app-wide by `Source/Features/Shared/Ui/SonikLookAndFeel`, installed as the JUCE default LookAndFeel in `SonikApplication::initialise()` (which also creates the single app-wide `juce::TooltipWindow`). Never restyle these stock widgets per-component; a bare widget automatically picks up the DESIGN.md chrome. If a component needs a bespoke LookAndFeel for other widgets, inherit from `sonik::ui::SonikLookAndFeel` (not `juce::LookAndFeel_V4`) so its chrome stays consistent.
