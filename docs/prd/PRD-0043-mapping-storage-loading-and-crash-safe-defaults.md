...
- The system ensures that two bundled default profiles ship inside the application binary: `behringer-ddm4000.json` (a full curated mapping for the Behringer DDM4000 with LED feedback) and `generic-midi.json` (an empty bindings profile used as a fallback for unknown devices).
...
- The system ensures that the bundled Behringer DDM4000 profile in `behringer-ddm4000.json` is curated from the reference `.tsi` in `midi_mappings/ddm4000/` and the Behringer DDM4000 user manual, covering the controls the mixer physically exposes: per-channel CUE button (4×), channel gain (4×), EQ high / mid / low + per-band kills (4×), channel fader (4×), crossfader, channel assign A/B buttons (4×), master/booth/headphone-volume knobs, headphone master/cue balance, and the menu/edit encoder (mapped to library scroll). LED feedback declarations are emitted for every lit control the DDM4000 exposes via MIDI out — channel CUE buttons and the channel assign A/B LEDs.
...
2. The constructor reads the two bundled JSON profiles from `BinaryData::behringer_ddm4000_json` and `BinaryData::generic_midi_json` (embedded by JUCE's `juce_add_binary_data` CMake helper).
...
4. The two resulting `Mapping` instances are stored in a `std::unordered_map<juce::String, std::shared_ptr<const Mapping>>` keyed by a stable bundled-profile id (`"behringer-ddm4000"`, `"generic-midi"`).
...
2. For each file: read its contents into a `juce::String` via `juce::File::loadFileAsString()`. Empty files and files larger than 1 MB (a paranoid upper bound; the DDM4000 reference profile is well under 100 KB) are skipped with a logged warning.
...
5. The accepted `Mapping` is inserted into a separate `std::unordered_map<juce::String, std::shared_ptr<const Mapping>>` keyed by the file's stem name (e.g., `my-custom-ddm4000` for `my-custom-ddm4000.json`).
...
4. If no user profile matches, the store iterates `bundledProfiles`. The Behringer DDM4000 bundled profile matches manufacturer "Behringer" and product name regex matching "DDM4000.*".
...
2. The UI calls `MappingStore::saveUserMapping(newMapping, "user-ddm4000-custom.json")` on the Message thread.
...
4. The store writes the serialised JSON to `~/Library/Application Support/Sonik/MidiMappings/user-ddm4000-custom.json.tmp` via `juce::File::replaceWithText`. On any I/O error, returns `SaveResult::IoFailure` with the underlying error.
...
- [ ] The system embeds `behringer-ddm4000.json` and `generic-midi.json` in the application binary via JUCE's `juce_add_binary_data` CMake helper, with source files under `Resources/MidiMappings/`.
...
- [ ] The system bundled DDM4000 profile (`behringer-ddm4000.json`) declares `device.match.manufacturer: "Behringer"` and `device.match.midiName: ".*DDM4000.*"` (regex tolerant of OS-version suffixes like " (MIDI)" or " Port 1").
- [ ] The system bundled DDM4000 profile contains at minimum these binding targets, covering each of the four mixer channels and the global mixer section: `mixer.channel.{A,B,C,D}.cue` (toggle), `mixer.channel.{A,B,C,D}.gain` (continuous, soft-takeover), `mixer.channel.{A,B,C,D}.eq.high`, `.eq.mid`, `.eq.low` (continuous, soft-takeover), `mixer.channel.{A,B,C,D}.eq.killHigh`, `.killMid`, `.killLow` (toggle), `mixer.channel.{A,B,C,D}.fader` (continuous, soft-takeover), `mixer.channel.{A,B,C,D}.assignA`, `.assignB` (toggle), `mixer.crossfader`, `mixer.master.gain`, `mixer.booth.gain`, `mixer.headphones.gain`, `mixer.headphones.cueMix`, `library.scroll.up`, `library.scroll.down` (driven by the DDM4000 menu/edit encoder via `signedBitDelta`). The set of target IDs the registry must expose is defined in PRD-0042.
- [ ] The system bundled DDM4000 profile declares no `modifier` entries. Modifier-layer authoring (PRD-0046) is supported by the schema for user-authored profiles bound to controllers with a dedicated SHIFT/ALT surface; the DDM4000 has no such surface in v1.
- [ ] The system bundled DDM4000 profile declares an outbound `feedback` block for every LED-capable control on the DDM4000: the four channel CUE buttons (binary, sourced from `mixer.channel.{A,B,C,D}.cueEnabled`) and the eight channel-assign LEDs (binary, sourced from `mixer.channel.{A,B,C,D}.assignA` / `.assignB`). Each `feedback` block specifies the outbound MIDI message (channel, status, data1, onValue, offValue) and the `valueTree` key sourcing the on/off state.
